...
export default function App() {
  // Auth States
  const [token, setToken] = useState(localStorage.getItem('clouddrive_token') || null);
  ...
  // Fetch files from backend (secured with JWT)
  ...
  // Handle connection check and initial file fetch
  useEffect(() => {
    checkConnection();
    if (token) {
      loadFiles();
  ...
  const copyLink = (url, key) => {
    ...
    setCopiedKey(key);
    showToast('Secure user-presigned link copied (expires in 1hr).', 'success');
  ...
  return (
    <div className="min-h-screen monochrome-grid relative flex flex-col font-sans text-zinc-300 selection:bg-white selection:text-black">
            ...
            {token && user && (
              <div className="flex items-center gap-3 pr-3 border-r border-zinc-900">
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1 text-xs text-zinc-400 hover:text-white bg-zinc-900/50 hover:bg-zinc-800 border border-zinc-800 rounded-lg transition-colors"
                  title="Logout"
                >
      ...
      {/* Authentication Screen / Main Dashboard conditional rendering */}
      <AnimatePresence mode="wait">
        {!token ? (
          // --- AUTHENTICATION CARD SCREEN ---
          <motion.div
            key="auth-screen"
          ...
          >
            <div className="w-full max-w-md glass-panel p-8 rounded-3xl flex flex-col gap-6 shadow-2xl relative overflow-hidden">
              ...
              <div className="flex flex-col items-center gap-2 text-center">
                ...
                <h3 className="font-display font-bold text-white text-lg tracking-wide">Secure Storage Gateway</h3>
                <p className="text-xs text-zinc-500 max-w-xs leading-relaxed">
                  Authentication is required to access your private sandboxed files and storage directories.
                </p>
              </div>
              ...
              <form onSubmit={handleAuthSubmit} className="flex flex-col gap-4">
                ...
                >
                  {authLoading ? (
                    ...
                    <span>{authMode === 'login' ? 'ENTER_SECURE_DRIVE' : 'CREATE_SECURE_DRIVE'}</span>
                  )}
                </button>
              </form>
          ...
          // --- SECURE STORAGE DASHBOARD SCREEN ---
          <motion.div
            ...
            <div className="lg:col-span-4 flex flex-col gap-6">
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 pt-2 border-t border-zinc-900">
                  <div>
                    <span className="text-[10px] text-zinc-500 font-mono uppercase">Your Files</span>
              ...
              >
                ...
                <ul className="space-y-3.5 text-xs">
                  <li className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-zinc-200">PostgreSQL Metadata DB</p>
                      <p className="text-[10px] text-zinc-500">Relational file tracking & DB migrations</p>
                    </div>
                  </li>
                  <li className="flex items-center gap-3">
                    <div className="w-4 h-4 rounded-full bg-white/10 flex items-center justify-center text-[9px] text-white font-bold">
                    ...
                    <div>
                      <p className="font-medium text-zinc-200">BCrypt Password Hash</p>
                      <p className="text-[10px] text-zinc-500">Salted password storage in pg DB</p>
                    </div>
                  </li>
                  <li className="flex items-center gap-3">
                    <div className="w-4 h-4 rounded-full bg-white/10 flex items-center justify-center text-[9px] text-white font-bold">
                    ...
                    <div>
                      <p className="font-medium text-zinc-200">JSON Web Tokens (JWT)</p>
                      <p className="text-[10px] text-zinc-500">Cryptographically signed authorization headers</p>
                    </div>
                  </li>
                  <li className="flex items-center gap-3">
                    <div className="w-4 h-4 rounded-full bg-white/10 flex items-center justify-center text-[9px] text-white font-bold">
                    ...
                    <div>
                      <p className="font-medium text-zinc-200">Isolated User Buckets</p>
                      <p className="text-[10px] text-zinc-500">Files nested inside users/{"{"}user_id{"}"}/ paths</p>
                    </div>
                  </li>
                  <li className="flex items-center gap-3">
                    <div className="w-4 h-4 rounded-full bg-white/10 flex items-center justify-center text-[9px] text-white font-bold">
                    ...
                    <div>
                      <p className="font-medium text-zinc-200">S3 Presigned Access URLs</p>
      ...
      <footer className="border-t border-zinc-900/60 bg-zinc-950/20 py-6 text-center font-mono text-[10px] text-zinc-600">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-zinc-500">AES-256 JWT ENCRYPTED TRANSIT</span>
          </div>
        </div>
      </footer>
